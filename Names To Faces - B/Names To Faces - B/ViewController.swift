...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let json = defaults.object(forKey: "people") as? Data {
            let decoder = JSONDecoder()
            if let decoded = try? decoder.decode([Person].self, from: json) {
                people = decoded
            } else {
    ...
    func save() {
        ...
        if let json = try? encoder.encode(people) {
            let defaults = UserDefaults.standard
            defaults.set(json, forKey: "people")
        } else {
